...
| `POST /stream/analyzeScenario` | **Multi-agent analyzeScenario over SSE** — same 3-specialist supervisor flow as `/ai/analyzeScenario` but streams `streamTools()` events (`turn_start`, `text`, `tool_call_start`, `tool_call_result`, `done`) as they happen. Chat UIs render tool badges (`invoke_contract-lookup…`) and coordinator prose live instead of blocking on the full trace. |
