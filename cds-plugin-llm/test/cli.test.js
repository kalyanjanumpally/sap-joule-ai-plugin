...
test('providerFactory: knows every provider kind', () => {
  assert.deepEqual(PROVIDER_KINDS.sort(), ['anthropic', 'genai-hub', 'groq', 'ollama', 'openai-compatible']);
});
...
test('providers: --json returns structured rows', async () => {
  ...
  assert.deepEqual(parsed.map(r => r.kind).sort(), ['anthropic', 'genai-hub', 'groq', 'ollama', 'openai-compatible']);
});
